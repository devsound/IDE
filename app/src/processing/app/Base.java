...
//      }
  ...
  public void rebuildImportMenu(JMenu importMenu, final Editor editor) {
    ...
    */
    ...
    try {
      //getBoardPreferences XXX
      Map<String, String> boardPreferences = Base.getBoardPreferences();
      addLibraries(importMenu, new File(librariesFolder.toString() + File.separator + boardPreferences.get("build.tools")));
    } catch (IOException e) {
      e.printStackTrace();
    }
  ...
  public void rebuildModeMenu(JMenu menu, final Editor editor) {
    //System.out.println("rebuilding boards menu");
    menu.removeAll();
    ButtonGroup group = new ButtonGroup();
    java.util.List<String> modes = Arrays.asList("Serial", "DFU");
    for (String mode : modes) {
        AbstractAction action = 
          new AbstractAction(mode) {
            public void actionPerformed(ActionEvent actionevent) {
              Preferences.set("progmode", (String)getValue("target"));
		      //JOptionPane.showMessageDialog(null, (String)getValue("target"), Preferences.get("progmode"), JOptionPane.INFORMATION_MESSAGE);
             }
          };
        action.putValue("target", mode);
        JMenuItem item = new JRadioButtonMenuItem(action);
        if (mode.equals(Preferences.get("progmode"))) {
          item.setSelected(true);
        }
		//JOptionPane.showMessageDialog(null, "relöl", Preferences.get("progmode"), JOptionPane.INFORMATION_MESSAGE);
        group.add(item);
        menu.add(item);
    }
  }
          ...
          if (new File(path).exists()) {
            ...
            showWarning(_("Sketch Does Not Exist"),
                        _("The selected sketch no longer exists.\n" +
                          "You may need to restart DevSound to update\n" +
...
//  }
  ...
  static public File getSketchbookLibrariesFolder() {
    Map<String, String> boardPreferences = Base.getBoardPreferences();
    File libdir = new File(getSketchbookFolder(), "libraries" + File.separator + boardPreferences.get("build.tools"));
    if (!libdir.exists()) {
      try {
        libdir.mkdirs();
