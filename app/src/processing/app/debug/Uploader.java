...
public abstract class Uploader implements MessageConsumer  {
  ...
  public abstract boolean uploadUsingPreferences(Sketch sketch, String buildPath, String className, boolean usingProgrammer)
  ...
  public void message(String s) {
    ...
    // suppress a bunch of dfu-util output
    || m.equals("dfu-util 0.8")
    || m.equals("Copyright 2005-2009 Weston Schmidt, Harald Welte and OpenMoko Inc.")
    || m.equals("Copyright 2010-2014 Tormod Volden and Stefan Schmidt")
    || m.equals("This program is Free Software and has ABSOLUTELY NO WARRANTY")
    || m.equals("Please report bugs to dfu-util@lists.gnumonks.org")
    || m.equals("Invalid DFU suffix signature")
    || m.equals("A valid DFU suffix will be required in a future dfu-util release!!!")
    || m.equals("No DFU capable USB device available")
    || m.equals("")
    || m.equals("Opening DFU capable USB device...")
    || m.startsWith("ID de50:")
    || m.startsWith("Run-time device DFU version")
    || m.equals("Claiming USB DFU Interface...")
    || m.startsWith("Setting Alternate Setting #")
    || m.startsWith("Determining device status:")
    || m.equals("dfuIDLE, continuing")
    || m.startsWith("DFU mode device DFU version")
    || m.startsWith("Device returned transfer size")
    || m.equals("Copying data from PC to DFU device")
    || m.equals("unable to read DFU status after completion")
    || m.startsWith("Download ")
    || m.startsWith("Download\t")))
		s = "";

    if(m.equals("Download done.")) {
      sketch.setCompilingProgress(100);
    } else if(m.startsWith("Download\t")) {
      sketch.setCompilingProgress(Integer.parseInt(m.substring(37, 40).trim()));
    }
