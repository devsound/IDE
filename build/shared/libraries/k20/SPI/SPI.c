...
void spi_io(int pin, void *tx, size_t tx_len, void *rx, size_t rx_len, bool last) {
  ...
  spi_queue_xfer(&ctx, pcs, (const uint8_t *)tx, (uint16_t)tx_len, (uint8_t *)rx, (uint16_t)rx_len, NULL, NULL, !last);
  while(spi_is_xfer_active());
}

uint8_t spi_iobyte(int pin, uint8_t val, bool last) {
  uint8_t volatile rx;
  spi_io(pin, &val, 1, (uint8_t*)&rx, 1, last);
  return rx;
}