...
static void i2c_start_transaction(void) {
  ctx.index = 0;
  if (ctx.cur->direction == I2C_READ) {     // if this is a read transaction
    ctx.state = I2C_STATE_RX_START;
    I2C0.c1.raw = ((struct I2C_C1) {
      .iicen = 1,
      .iicie = 1,
      .mst = 1,
      .tx = 1,
      .txak = ctx.cur->length == 1
    }).raw; // transmit acknowledge already when only one byte to receive
...
static void i2c_end_transaction() {
  // send STOP condition if requested or nothing left to send, otherwise repeat START condition
  if (ctx.cur->next == NULL || ctx.cur->stop == I2C_STOP) I2C0.c1.mst = 0;
  else I2C0.c1.rsta = 1;
}

// start the next transaction
static void i2c_transaction_next(enum i2c_result result) {
  ...
  if (ctx.cur == NULL) ctx.state = I2C_STATE_IDLE;
  else i2c_start_transaction();

  // invoke callback if specified
  if (prev->cb != NULL) prev->cb(result, prev);
}
  ...
  switch (ctx.state) {
    case I2C_STATE_TX:
      if (ctx.index < ctx.cur->length) {
        ...
        if (I2C0.s.rxak) result = I2C_RESULT_NACK; // if not acked, report it
        i2c_end_transaction();
        i2c_transaction_next(result);
      }
      break;
    case I2C_STATE_RX_START:
      if (I2C0.s.rxak) {    // report premature nack
        ...
        (void)I2C0.d; // throw away the first byte read from the device.
      }
      break;
    case I2C_STATE_RX:
...
void i2c_init(enum i2c_rate rate) {
  ...
  if (rate < 0 || rate >= sizeof(f)) rate = I2C_RATE_100;
  I2C0.f.raw = f[rate];
