...
static void spi_stop_xfer(void) {
  SPI0.mcr.raw = ((struct SPI_MCR) {
    .mstr = 1,
    .dconf = SPI_DCONF_SPI,
    .rooe = 1,
...
int spi_queue_xfer(struct spi_ctx *ctx, enum spi_pcs pcs, const uint8_t *txbuf, uint16_t txlen, uint8_t *rxbuf, uint16_t rxlen, spi_cb *cb, void *cbdata, bool cont) {
  return spi_queue_xfer_sg(&ctx->ctx, pcs, sg_init(&ctx->tx_sg, (void *)txbuf, txlen), sg_init(&ctx->rx_sg, rxbuf, rxlen), cb, cbdata, cont);
}

int spi_queue_xfer_sg(struct spi_ctx_bare *ctx, enum spi_pcs pcs, struct sg *tx, struct sg *rx, spi_cb *cb, void *cbdata, bool cont) {
  if(ctx->queued) return 1;
  ...
  *ctx = (struct spi_ctx_bare) {
  ...
  if(rx_len > tx_len) ctx->rx_tail = rx_len - tx_len;
...
again:
  ...
  for (;;) {
    ...
    if(flags.rfdf && spi_ctx->rx) {
      ...
      ctx->queued = false;
      spi_ctx = ctx->next;
      if(spi_ctx != NULL) spi_start_xfer();
      else spi_stop_xfer();
      crit_exit();
      if(ctx->cb != NULL) ctx->cb(ctx->cbdata);
      goto again;
    } else {
      break;
    }
