...
void serial1_begin(uint32_t baudrate, enum SERIAL_FMT_e format) {
  ...
  UART1.c1.raw = ((struct UART_C1_t) {
    .ilt = 1,
		.pe = (format == SERIAL_FMT_8E1
		     ||format == SERIAL_FMT_8O1),
		.pt = (format == SERIAL_FMT_8O1),
		.m  = (format == SERIAL_FMT_8N2
		     ||format == SERIAL_FMT_8E1
				 || format == SERIAL_FMT_8O1)
  }).raw;
