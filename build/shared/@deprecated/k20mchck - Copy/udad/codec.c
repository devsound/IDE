...
void codec_begin(CODEC_CSB_e codec_address) {
  ...
  codec_interface((struct CODEC_INTERFACE_t) {
    .format = CODEC_FORMAT_I2S,
    .iwl    = CODEC_IWL_32BIT   // Lets always use 32-bit
  });
  // Default all volumes to zero
  codec_llinein((struct CODEC_LINEIN_t) { .volume = 0 });
  codec_rlinein((struct CODEC_LINEIN_t) { .volume = 0 });
  codec_lheadout((struct CODEC_HEADOUT_t) { .volume = 0 });
  codec_rheadout((struct CODEC_HEADOUT_t) { .volume = 0 });
  // Select output from DAC
  codec_apath((struct CODEC_APATH_t) { .dacsel = 1 });
  // Enable ADC
  codec_dpath((struct CODEC_DPATH_t) { });
  // Power down nothing (enable everything)
  codec_powerdown((struct CODEC_POWERDOWN_t) { });
