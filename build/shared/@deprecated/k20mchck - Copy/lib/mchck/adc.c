...
static volatile struct adc_ctx adc_ctx; // stg: agressive optimization requires volatile or .active always reads as 1 in adc_sample_start
...
adc_init(void)
{
        ...
        adc_ctx.stat.cb = adc_calibrate_cb;
        adc_ctx.stat.active = 1;
...
{
        if (adc_ctx.stat.active)
                return (-1);

        /* XXX check for previous conversion running */
        adc_ctx.stat.cb = cb;
        adc_ctx.stat.cbdata = cbdata;
...
adc_sample_abort(void)
{
        ...
        if (adc_ctx.stat.active) {
                adc_result_cb_t *cb = adc_ctx.stat.cb;
                void *cbdata = adc_ctx.stat.cbdata;

                adc_ctx.stat.active = 0;
                crit_exit();
                cb(0, 1, cbdata);
                return (0);
        } else {
